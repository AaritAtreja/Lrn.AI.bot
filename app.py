...
async def get_context_chunks_and_metadata(question: str, administrator: bool):
    matches = await asyncio.to_thread(retrieve_relevant_chunks, question)
    context_chunks = []
    system_image_paths = []
    administrator_metadata_lines = []
    seen_metadata = set()
    ...
    for match in matches:
        ...
        if administrator:
            ...
            if meta_key not in seen_metadata:
                ...
                administrator_metadata_lines.append(f"PDF: {pdf_name}, Index: {index}, URL: {url}")
        if url != "nAn":
            system_image_paths.append(url)
                    
    context_text = "\n\n".join(context_chunks)
    return context_text, system_image_paths, administrator_metadata_lines
...
async def prepare_image_messages(system_image_paths, user_image_paths, administrator):
    messages = []
    if system_image_paths:
        msgs = await asyncio.to_thread(append_images_to_user_messages, system_image_paths)
        messages.extend(msgs)
    if administrator and user_image_paths:
...
async def answer_question(question: str, user_image_paths, administrator: bool = False):
    context_task = asyncio.create_task(get_context_chunks_and_metadata(question, administrator))
    website_task = asyncio.create_task(get_website_summaries(question))
    
    context_text, system_image_paths, administrator_metadata_lines = await context_task
    ...
    system_messages = [{
        "role": "system",
        "content": (
            "You are an AI assistant designed to help students and elderly by providing accurate and reliable answers. "
            "Your responses should be based strictly on the information available in the vector store. "
    ...
    image_messages = await prepare_image_messages(system_image_paths, user_image_paths, administrator)
    ...
    if administrator and administrator_metadata_lines:
        metadata_str = "\n".join(administrator_metadata_lines)
...
st.markdown(
    ...
    .custom-right-sidebar a {
        color: var(--primary-color);
        text-decoration: none;
        font-size: 0.9rem;
        font-weight: 500;
    }
    .custom-right-sidebar a:hover {
...
st.title("Digital Equity Empowerment Program (DEEP) Chatbot")
...
if prompt := st.chat_input("Enter your question"):
    ...
    # Process the question asynchronously. Here we assume no additional user images and administrator flag is False.
    try:
        answer_text, metadata = asyncio.run(answer_question(prompt, user_image_paths=[], administrator=False))
    except Exception as e:
